...
async def on_ready():
    ...
    await Bot.change_presence(status=discord.Status.online, activity=discord.Game(g[0]))
...
async def on_message(message):
    ...
    if Stringo[0:1] == '~': #checks if its a command or normal msg
        ...
        if message.content.startswith("~help"): #if command == ~help
            await message.channel.send("Implement commands: ~help, ~summary, ~search, ~reference, ~random, ~image, ~categories, ~link, ~test, ~page, ~content, ~section, ~sum, ~settings, ~simple, and ~url") #sends the commands
            ...
            if dicto.get(m) != None: #checks if the user has set a language for the server
                languag = dicto.get(m) #it returned none, so it gets the value

            WikiClass = wikipediaapi.Wikipedia(language=languag, extract_format=wikipediaapi.ExtractFormat.WIKI) #initiates the wikipedia class and sets the language depending on the server choice
            ...
            WikiClass = wikipediaapi.Wikipedia(language=languag, extract_format=wikipediaapi.ExtractFormat.WIKI) #initiates the wikipedia class
            ...
            if dicto.get(m) != None: #checks if the user has set a language for the server
                languag = dicto.get(m) #it returned none, so it has a value
                wikipedia.set_lang(languag) #assigns the value
            ...
            if len(Stringo) == 9: #checks if the user just requested the settings
                await message.channel.send("Set server's language with '~settings {}', The languages are: English = en, Swedish = sv, German = de, French = fr, Dutch = nl, Russian = ru, Italian, = it, Spanis = es, Polish = pl, Japanese = ja, and Arabic = ar.") #sends this msg
            
            else:
                
                Languages = ["en", "ceb", "sv", "de", "fr", 'nl', "ru", "it", "es", "pl", "ja", "ar"] #sets the languages
